...
import { useTask, useStopTask, useDeleteTask, useStartTask } from '@/lib/queries/task';
...
export function TaskDetailsPage() {
  ...
  const { mutate: startTaskMutation, isPending: isStartingTask } = useStartTask();
  ...
  const handleStartTask = async () => {
    ...
    try {
      ...
      notifications.show({
        ...
        message: t('tasks:notifications.taskStartedMessage', { name: task.name }),
        color: 'green'
      });
      refetch();
    } catch (error) {
  ...
  const handleStopTask = () => {
    ...
    modals.openConfirmModal({
      ...
      labels: { confirm: t('tasks:modals.stopTask.confirm'), cancel: t('common:cancel') },
        ...
        stopTask(task.id, {
          onSuccess: () => {
            notifications.show({
              ...
              message: t('tasks:notifications.taskStoppedMessage', { name: task.name }),
              color: 'yellow'
            });
          },
          onError: (error) => {
  ...
  const handleDeleteTask = () => {
    ...
    modals.openConfirmModal({
      ...
      labels: { confirm: t('tasks:modals.deleteTask.confirm'), cancel: t('common:cancel') },
        ...
        deleteTask(task.id, {
          onSuccess: () => {
            notifications.show({
              ...
              message: t('tasks:notifications.taskDeletedMessage', { name: task.name }),
              color: 'green'
            });
            navigate('/tasks');
          },
  ...
  if (isLoading) {
    return (
      <PageLayout title={t('tasks:taskDetails')} description={t('tasks:taskDetailsDescription')}>
    ...
    return (
      <PageLayout title={t('tasks:taskDetails')} description={t('tasks:taskDetailsDescription')}>
        <Center style={{ height: 300, flexDirection: 'column' }}>
          ...
          <Button variant="outline" leftSection={<Icons.ArrowLeft size={16} />} onClick={handleBack}>
            {t('tasks:backToTasks')}
          </Button>
        </Center>
      </PageLayout>
    ...
    >
      <Stack gap="lg">
              ...
              <Stack gap="xs">
                ...
                <Group>
                  ...
                  <Badge color="blue" radius="sm" style={{ textTransform: 'capitalize' }}>
                    {t(`tasks:taskTypes.${task.taskType}`)}
                  </Badge>
                </Group>
                ...
                <Group>
                  <Icons.Clock size={16} />
                  <Text size="sm">
                    {t('tasks:createdAgo', { time: formatDistanceToNow(new Date(task.createdAt)) })}
                  </Text>
                </Group>
                ...
                {task.startTime && (
                  <Group>
                    <Icons.Clock size={16} />
                    <Text size="sm">
                      {t('tasks:startedAgo', { time: formatDistanceToNow(new Date(task.startTime)) })}
                    </Text>
                  </Group>
                )}
                ...
                {task.endTime && (
                  <Group>
                    <Icons.Clock size={16} />
                    <Text size="sm">
                      {t('tasks:endedAgo', { time: formatDistanceToNow(new Date(task.endTime)) })}
                    </Text>
                  </Group>
                )}
            ...
            <Grid.Col span={{ base: 12, sm: 6 }}>
              ...
              >
                ...
                {task.metrics && task.metrics.processingFps && (
                  <Group mt="md">
                    <Text size="sm" fw={500}>{t('tasks:processingSpeed')}:</Text>
                ...
                {task.metrics && task.metrics.objectsCounted && (
                  <Group mt="xs">
                    <Text size="sm" fw={500}>{t('tasks:objectsCounted')}:</Text>
        ...
        <Grid gutter="lg">
              ...
              <Card p="lg" radius="md" withBorder>
                <Center style={{ height: 400, flexDirection: 'column' }}>
                  ...
                  <Text size="lg" c="dimmed" mt="md">
                    {task.status === 'pending' && t('tasks:videoWillAppearWhenStarted')}
                    {task.status === 'failed' && t('tasks:taskFailedNoVideo')}
                    {task.status === 'completed' && t('tasks:taskCompletedVideoEnded')}
                    {task.status === 'stopped' && t('tasks:taskStoppedVideoEnded')}
                  </Text>
                </Center>
              </Card>
            )}
          ...
          {task.status === 'running' && task.metrics && (
